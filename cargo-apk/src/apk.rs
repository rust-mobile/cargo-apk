...
use ndk_build::manifest::{Activity, AndroidManifestInput, IntentFilter, MetaData};
...
impl<'a> ApkBuilder<'a> {
    pub fn from_subcommand(
        ...
        if let AndroidManifestInput::FromToml(ref mut android_manifest) = manifest.android_manifest
        {
            ...
            if android_manifest.application.activities.is_empty() {
                android_manifest
                    .application
                    .activities
                    .push(Activity::default());
            }
            let activity = &mut android_manifest.application.activities[0];

            // Add a default `MAIN` action to launch the activity, if the user didn't supply it by hand.
            if activity.intent_filter.iter().all(|i| {
                i.actions
                    .iter()
                    .all(|f| f.name != "android.intent.action.MAIN")
            }) {
    ...
    pub fn build(&self, artifact: &Artifact) -> Result<Apk, Error> {
        ...
        if let AndroidManifestInput::FromToml(ref mut manifest) = android_manifest {
            ...
            manifest.application.activities[0].meta_data.push(MetaData {
